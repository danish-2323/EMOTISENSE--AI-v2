...
export default function StudyCoach({ sessionActive, liveData, plan }) {
  ...
  async function fetchNudge() {
    setLoading(true)
    try {
      const res  = await fetch(`https://emotisense-e6z2.onrender.com/study/coach/nudge?subject=${encodeURIComponent(plan?.subject || '')}&state=${encodeURIComponent(liveData?.study_state || 'Distracted')}`)
